...
ocpp_proto = Proto("ocpp2.0", "Open Charge Point Protocol v2.0 Dissector")

ocpp_proto.prefs.schemas20  = Pref.string("Path to OCPPv2.0 schemas:  ", "", "Path to OCPPv2.0 schemas")

-- Define fields for the protocol
local f_message_type = ProtoField.uint8("ocpp2.0.message_type", "Message Type", base.DEC)
local f_message_id = ProtoField.string("ocpp2.0.message_id", "Message ID")
local f_message_name = ProtoField.string("ocpp2.0.message_name", "Message Name")
local f_payload = ProtoField.string("ocpp2.0.payload", "Payload (JSON)")
local f_valid = ProtoField.bool("ocpp2.0.valid", "Valid?", base.NONE)
local f_ipv6 = ProtoField.bool("ocpp2.0.ipv6", "IPv6?", base.NONE)
...
local function validate_json(payload, schema_name)
    ...
    if success20 then
        ...
        return false, tostring(err20), 'None'
    end
end
...
function ocpp_proto.dissector(buffer, pinfo, tree)
    ...
    if is_valid then
        ...
        pinfo.cols.protocol = "OCPP 2.0"
        -- Create the protocol tree
        local subtree = tree:add(ocpp_proto, buffer(), "OCPP Non-Compliant Packet")
        subtree:add(ProtoField.string("ocpp2.0.error", "Error"), buffer(), tostring(validation_error))
        subtree:add(f_valid, is_valid):set_hidden(true)
